...
In this technique, the clients only ever talk to the primary node `P`.  Any time `P` receives a write request, that request is broadcast to all the backups nodes, which individually send their `ack` back to the primary.
...
Irrespective of the number of backups in this system, there are never more than four steps needed to complete a write
...
The primary node takes all the workload; thus, it not only becomes a bottleneck, but also does not allow for any horizontal scalability.
...
So, if you have a large number of backups, then write could potentially take a long time.

The point however of Chain Replication is to improve read throughput by directing all reads to the tail.
...
Comparing the Chain and P/B Replication curves, notice that if none of the requests are updates, then the performance of Chain and P/B replication is identical. The same is true when about 50% of the requests are updates.

Look at the Chain Replication curve.  Instead of descending in a gradually flattening curve, there is a hump at around the 10-15% mark.  This is where the benefits of Chain Replication can be seen.

By why should this improvement be seen at this particular ratio of writes to reads?

The answer here lies in understanding how the division of labour between the head and tail processes in Chain Replication distributes process workload.  All write requests are handled by the head and all read requests are handled by the tail.  According to the research done by Renesse and Schneider, their experiments showed that when 10-15% of the requests are writes, then this produces an equal workload between the head and tail processes.

It turns out that in practice, this is ratio of writes to reads is quite representative of many distributed systems *"out there in the wild"*.
...
Well, clients always need to know who to contact in the first place, and this role is usually performed by some sort of proxy.

In the Chain Replication system:
...
So, in both situations, it is necessary to have some sort of coordinating process whose job it is to know who all the replicas are, and what role they are playing at any given time.  This coordinator is often completely internal to the system.

> ***Assumption***  
> The following discussion makes the following assumptions:
> 
> * Not all the processes in our system will crash.  For a system containing `n` processes, we are relying on the fact that no more than `n-1` processes will ever crash.
> * The coordinator process is able to detect when a process crashes.
>
> However, we have not discussed how such an assumption could possibly be true because *"crashing"* could mean a variety of things: perhaps software execution has terminated, or the process execution continues but simply stops responding to messages, or responds very slowly...
> 
...
* Should a replica fail, the coordinator must reconfigure the remaining replicas such that the overall system keeps running (this will probably also involve attempting to restart the failed replica)
...
* Nominate a backup to act as the new primary
* Inform all clients to direct their requests to the new primary
* Attempt to restart the failed primary
* Inform the restarted primary that it is now functioning as a backup
* etc...
...
* Nominate the head's successor to act as the new head
* Inform all clients to direct their write requests to the new head
* Attempt to restart the failed head
* Inform the restarted head that it is now functioning as a backup
* etc...
...
If we go to all the trouble of implementing a system that replicates data across some number of backups, but use a single coordinator process to manage those replicas, then in some sense, we've actually taken a big backwards step because we've introduced a new *"single point of failure"*.

So, what steps can we take to be more tolerant of coordinator failure - simply replicate the coordinators?  In one data centre, or across multiple data centres?

But then how do keep the coordinators coordinated?

Do you have a coordinator coordinator?  If so, who coordinates the coordinator coordinator?
...
It is sobering to realise that if we wish to implement both strong consistency between replicas ***and*** fault tolerance (which was the problem we needed to mitigate in the first place), then ultimately, we are forced to rely upon some form of consensus protocol.

But consensus is ***hard*** and ***expensive*** to implement.  This difficulty might then be a good reason ***not*** to implement strong consistency.  It now looks very appealing to say *"If we can get away with a weaker form of consistency such as Causal Consistency, then maybe we should look at this option"*.

However, there are times when consensus really is needed.
