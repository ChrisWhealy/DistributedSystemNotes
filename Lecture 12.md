...
Looking back at our total order anomaly diagram, we can see that this problem raises the issues of maintaining data consistency across replicas.
...
A further problem is that neither of the situations described above can be considered wrong.  Both runs of the system are valid and correct, and neither demonstrate a Total Order delivery.
...
Process `R1` is the only process delivering messages; therefore, it can never be in violation of the Totally Ordered Delivery rule!  This gives process `R1` a lot of control over when to deliver messages.  In fact, process `R1` can decide what is meant by *"Total Order"* by controlling when it delivers  messages.  In fact, `R1` could deliver messages in different orders across different runs and still not violate the safety property of Totally Ordered Delivery.

If process `R1` did change the delivery order from one run to the next, then it would be violating a property called ***Determinism***

## Determinism

Determinism is a property that relates to multiple runs of a system to each other.
...
When there's only one replica, it will establish its own total order; however, when we have multiple replicas, we need each replica to behave with the same, consistent total order so that all the clients think they are dealing with a single system that does not have multiple replicas.
...
The client sends the message `x=5` to `R1`, but this value is not replicated across to `R2`.  So, when the client next queries the value of `x`, it gets back the unexpected result of `undefined` (in other words *"Who's x?"*)
...
However, `R1` now sends out some synchronising messages, but `R2` delivers these messages in the wrong order.  After the first message is delivered, the client's balance is no incorrectly shown to be \$10 overdrawn.  If at this point in time, some automated balance checking process such as `C2` queries the client's balance, it will get the incorrect impression that this client has been spending too much money.

> ***Aside***  
> Wells Fargo Bank actually got in trouble for doing exactly this.  They deliberately reordered transactions in order to maximise their overdraft fees.
...
In this situation, all the events happen in chronological order such that the ***happens before*** relation is never violated.  However, but replica `R2` is missing an event in its causal history and consequently gives the wrong answer to a query.
...
* Client `C2` performs a balance enquiry against `C1` who accurately reports it as \$100
...
Here the idea is pretty straight-forward and has been around since the 1970's.  We pick a system to act as the primary, and the other systems act as backups. The advantages of this system are that:
...
In this scenario, clients only ever interact with the primary.  When a client message arrives, the primary replicates that messages to all the replicas, each of which must send back an `ack` to the primary.  

![Primary Backup Replication 1](./img/L12%20Primary%20Backup%20Replication%201.png)

Only when the primary receives `ack`s from all its replicas does it first deliver the message to itself, then send an `ack` back to the client.  This is known as the ***Commit Point***.
...
* Its slow because not only is the primary is the bottleneck, but the system's minimum response time can be no faster than the maximum `ack` response time from any one of the backups
* There is no horizontal scaling in case the primary becomes overloaded
* It cannot help with data locality since, by definition, there is only one primary

But could we do better?

Yes, we could redirect all reads to the backups and leave the primary to handle only writes.  

On the surface, this looks like a good solution, but upon closer examination, we discover that could end up with some weird timing issues and potentially start reading ***data from the future***.

How so?

If a write is issued to the primary, that write is replicated to all the backups.  However, the backups never send their `ack`s back to the client - only the primary receives these `acks`.  Only when the primary has received `ack`s from all the backups does it then send an `ack` to the client.

This means that if we direct a read request to a backup for data that has just been updated, but before the `ack` has reached the client (via the primary), then we will effectively be reading ***data from the future***
...
In chain replication, the backup sends its `ack` directly to the client.  This then means that should the client wish to make a subsequent read, it can direct that read to the backup from which it received the `ack`.

![Chain Replication](./img/L12%20Chain%20Replication.png)

This is a relatively new strategy that was first published by Robbert van Renesse and Fred Schneider in a 2004 paper called ["Chain replication for Supporting High Throughput Abd Availability"](https://www.cs.cornell.edu/home/rvr/papers/OSDI04.pdf)
...
The "Head" only handled writes and the "Tail" only handles reads.

The division of labour is now:

* All writes are handled first by the "Head" process
* The "Head" then replicates the write instruction down the chain of replicas terminating at the "Tail"
* When the "Tail" process completes the write, we know it is the last link in the chain, so it sends the `ack` to the client
* The "Tail" process then always handles read requests

***Q:***&nbsp;&nbsp; But the response time experienced by the client will now be the ***sum*** of the time taken for each process to complete the write and propagate it to the next link in the chain.  How can this then be described as *High Throughput*?
...
If we have a more or less equal mix of writes and reads, then in theory, Chain Replication will outperform Primary Backup Replication.  But this depends on the ratio of reads and writes.  For systems the perform mostly writes, then Primary Backup Replication will probably achieve higher throughput, but for systems that perform mostly reads, Chain Replication will probably achieve higher throughput.

### Chain Replication: Drawbacks

In Primary Backup replication, when the primary performs a write, the replication messages are sent out in parallel and the longest wait time corresponds to the slowest individual backup to send out its `ack`.  So not matter how many backups you have, the worst-case scenario will never be worse than the slowest backup.

However, in Chain Replication, the length of the backup chain defines the overall response time for a write to complete.  As the chain length increases, so the overall all response time increases because write replication happens sequentially down the length of the chain.
...
Both of these strategies are commonly used; however, the decision as to which one will work best for you is governed primarily on the balance you expect between reads and writes.

Generally speaking, a write takes approximately four times as long as a read; therefore, before Primary Backup Replication become a good choice, you should be designing a system in which you expect to receive at least a 4:1 ratio of reads:writes.  I.E. You expect to receive at least four reads requests for every write request, thus the time take for each roughly balances.
