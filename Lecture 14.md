...
* ***Coordinators - A Single Point of Failure?***
    ...
    ***A:***&nbsp;&nbsp; Yes, but if you really need strong consistency, then this is the type of approach you will need to take.
    ...
    Alternatively, you might decide that the extra overheads of time, expense and complexity are sufficiently high that you could tolerate a weaker form of consistency.
...
At the end of the previous lecture, we mentioned that in the in the original Chain Replication paper, one of the first things Renesse and Schneider state is that *"We assume the coordinator doesn't fail!"* &mdash; and they then admit that this is an unrealistic assumption.  They then go on to describe how in their tests, they had a set of coordinator processes that were able to behave as a single process by running a consensus protocol between them.

### When Do You Need Consensus?

Assuming we have a set of processes that must all act in some coordinated fashion, then we are faced with the following set of well-known problems:
...
1. ***The Group Membership Problem*** or ***Failure Detection Problem***  
    A group processes must agree either on their own mutual state, or the state of some other group of processes, whilst at the same time, any one of these processes could be starting, crashing, going slow or restarting
...
1. ***The Distributed Transaction Commit Problem***  
    All processes jointly contribute towards some sort of transaction.  It must then be agreed upon as to whether that transaction should be committed or aborted
...
What makes this really hard is the fact that faults can occur: namely ***crash faults*** or ***omission faults***.  But even in the somewhat simply case where we only need to deal with crash faults, this problem is still really hard.
...
The problem is that, in reality, it turns out to be impossible for a consensus algorithm to satisfy all three of these properties (at least in an asynchronous network model where processes can fail).
...
The problem is that if we compromise on either the agreement or validity properties, then we will have a system that fulfils the requirements, but will turn out not to be very useful.  Consequently, consensus algorithms must compromise on termination.
...
Nothing, nada, nix.  But this is a risk we need to take.
...
1. The proposal number must be ***unique***.  
    If the Paxos consensus algorithm is being implemented by multiple proposers, then before the algorithm starts, each of these processes must already have agreed upon how they will maintain proposal number uniqueness (for instance, one proposer might use even numbers and the other, odd).
...
At this point, we reached something of a milestone because we now have a majority of acceptors who have all agreed that they will respond to an as yet unsent value, that we will label with this particular proposal number.  In addition to this, there can now never be a majority of acceptors who have promised to pay attention to any proposal number less than `5`. (A minority might still agree on some lower proposal number, that is now of no consequence.)
...
* When an acceptor gets a `prepare(n)` message its asks *"Have I already agreed to ignore messages with the proposal number `n`?"*
...
* Once a proposer has received `promise(n)` messages from the majority of acceptors, it sends an `accept(n,val)` message to the majority of acceptors, where `n` is the agreed upon proposal number and `val` is the proposed value upon which consensus is sought.  
    (Again, we've skipped over some more details here that we'll need to revisit in the next lecture)
